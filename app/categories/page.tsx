...
export default function CategoriesPage() {
  ...
  const handleDeleteCategory = (category: Category) => {
    ...
    if (categorySubcategories.length > 0) {
      dependencies.push(`${categorySubcategories.length} subcategories`);
    }
    if (categoryProducts.length > 0) {
      dependencies.push(`${categoryProducts.length} products`);
    }
  ...
  const handleDeleteSubcategory = (subcategory: SubCategory) => {
    ...
    if (subcategoryProducts.length > 0) {
      dependencies.push(`${subcategoryProducts.length} products`);
    }
  ...
  const handleBulkDeleteCategories = () => {
    ...
    setItemsToDelete(categoriesToDelete);
    setDeleteType('category');
    setShowDeleteDialog(true);
  };

  const handleBulkDeleteSubcategories = () => {
    ...
    setItemsToDelete(subcategoriesToDelete);
    setDeleteType('subcategory');
    setShowDeleteDialog(true);
  };
  ...
  const confirmDelete = async () => {
    setIsDeleting(true);
    try {
      const validItems = itemsToDelete.filter(
      ...
      if (validItems.length === 0) {
        toast({
          ...
          description: "All selected items have dependencies and cannot be deleted.",
          variant: "destructive",
        });
        return;
      }

      const ids = validItems.map((item) => item.id).join(",");
      const endpoint = deleteType === 'category' ? '/api/categories' : '/api/subcategories';
      ...
      if (response.ok) {
        ...
        if (deleteType === 'category') {
          setCategories((prev) => prev.filter((cat) => !validItems.some(item => item.id === cat.id)));
          setSelectedCategories(new Set());
        } else {
          setSubcategories((prev) => prev.filter((sub) => !validItems.some(item => item.id === sub.id)));
          setSelectedSubcategories(new Set());
        }

        toast({
          title: "Items deleted successfully",
          description: `${result.summary.deleted} ${deleteType}${result.summary.deleted > 1 ? 's' : ''} deleted successfully.`,
        });

        if (result.errors && result.errors.length > 0) {
          toast({
            title: "Some items could not be deleted",
            description: `${result.errors.length} items failed to delete due to dependencies.`,
            variant: "destructive",
          });
        }
      } else {
  ...
  const selectAllSubcategories = () => {
    ...
    setSelectedSubcategories(new Set(currentSubcategories.map(sub => sub.id)));
  };
  ...
  const categoryBulkState = {
    allSelected: categories.length > 0 && selectedCategories.size === categories.length,
    someSelected: selectedCategories.size > 0 && selectedCategories.size < categories.length,
  };
  ...
  const subcategoryBulkState = {
    allSelected: getCurrentSubcategories().length > 0 && 
                 getCurrentSubcategories().every(sub => selectedSubcategories.has(sub.id)),
    someSelected: getCurrentSubcategories().some(sub => selectedSubcategories.has(sub.id)) &&
                  !getCurrentSubcategories().every(sub => selectedSubcategories.has(sub.id)),
  };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <BulkActionsToolbar
        ...
        allSelected={categoryBulkState.allSelected}
        someSelected={categoryBulkState.someSelected}
      />
      ...
      {selectedCategoryForManagement && (
        <>
          ...
          <BulkActionsToolbar
            ...
            totalCount={getCurrentSubcategories().length}
            onSelectAll={selectAllSubcategories}
            onDeselectAll={deselectAllSubcategories}
            onDelete={handleBulkDeleteSubcategories}
            allSelected={subcategoryBulkState.allSelected}
            someSelected={subcategoryBulkState.someSelected}
          />
